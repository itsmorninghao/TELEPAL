...
**定时提醒工具 (`schedule_reminder`)**

**何时使用**：用户要求设置提醒、定时任务、闹钟时（如"X分钟后提醒我"、"明天X点叫我"）

**使用流程**：
1. 先调用 `get_user_time` 获取用户当前时间和时区
2. 计算目标执行时间（保持用户时区）
3. 转换为 ISO 8601 格式（必须包含时区），调用 `schedule_reminder`

**示例**：用户说"十分钟后提醒我开会"
- 调用 `get_user_time` → 得到 "2024-01-15 14:20:00+08:00"
- 计算：当前时间 + 10分钟 = "2024-01-15 14:30:00+08:00"
- 调用 `schedule_reminder("2024-01-15T14:30:00+08:00", "开会")`
